...
const TrendingItem: FC<TrendingItemProps> = ({ repo, lastRepoElementRef = null }) => {
  ...
  return (
    <div 
      ...
      className="bg-white border border-gray-100 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 p-4 sm:p-6 mb-4 sm:mb-6"
    >
      ...
      {settings.showDescription && (
        <p className="text-gray-800 mt-2 text-left text-sm sm:text-base font-normal">
        ...
        {settings.showAvatar && (
          <div className="flex items-center">
            <img 
              ...
              className="w-7 h-7 sm:w-8 sm:h-8 rounded bg-gray-200 mr-2"
            />
            <span className="text-gray-800 font-medium text-sm sm:text-base">{repo.owner.login}</span>
          </div>
        )}
        {settings.showStars && (
      ...
      {settings.showTags && repo.topics && repo.topics.length > 0 && (
        <div className="mt-2 sm:mt-3 flex flex-wrap gap-2">
          {repo.topics.slice(0, 3).map((topic, idx) => (
            <span 
              key={topic} 
              className={`px-2 py-0.5 sm:py-1 rounded-full text-xs sm:text-sm font-medium ${tagColors[idx % tagColors.length]}`}
            >
              {topic}
            </span>
          ))}
